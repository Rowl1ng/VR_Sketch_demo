    ...
    void Update()
    {
        ...
        if (pressing == false && hideY == true)
        {
            ...
            // {
                ...
                // }
            // other_shapes = GameObject.FindGameObjectsWithTag("other_shape");
            shapes = SaveSketchLogic.GetComponent<SaveSketchLogic>().loadedObject;
            Debug.Log("other_shapes " + shapes.Length);
            if (shapes.Length != 0)
            {
                foreach (GameObject go in shapes)
                {
                    if (go.tag == "other_shape")
                    {
        ...
        if (pressing == false && RI > 0)
        {
            ...
            currLine.lmat.color = Color.white;
